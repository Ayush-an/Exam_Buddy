...
const Profile = () => {
  ...
  const handleProfileImageDelete = async () => {
    ...
    try {
      const res = await axios.delete(`${API_BASE_URL}/api/user/users/${userData._id}/profile-image`);
      if (res.data.status) {
        setProfileImage('');
  ...
  return (
    <div>
      <Navbar />
      <div className="min-h-screen pb-10 bg-gradient-to-br from-purple-200 to-purple-400">
        <div className="max-w-6xl p-6 mx-auto space-y-6">
          <div className="text-center">
            {profileImage && (
              <img src={profileImage} alt="Profile" className="w-32 h-32 mx-auto mb-4 rounded-full shadow-md" />
            )}
            <h2 className="text-2xl font-bold text-gray-800">
              {formData.firstName} {formData.lastName}
            </h2>
            <div className="flex flex-col items-center justify-center gap-2 mt-4">
              <label className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white transition bg-blue-600 rounded-md shadow cursor-pointer hover:bg-blue-700">
                <input
                  type="file"
                  accept="image/*"
              ...
              {profileImage && (
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium text-white transition bg-red-500 rounded-md shadow hover:bg-red-600"
                >
  ...
  {['firstName', 'lastName', 'mobile', 'email', 'dob', 'parentWhatsapp'].map((field) => (
    <div key={field} className="flex flex-col">
      ...
      <input
        ...
        className="p-3 border rounded-md shadow-sm outline-none focus:ring-2 focus:ring-purple-500"
      />
    </div>
  ))}
    ...
    Update Profile
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {latestPieData.length > 0 && (
              <div className="p-4 bg-white shadow-md rounded-2xl">
                <h3 className="mb-4 text-lg font-semibold text-center text-gray-700">Latest Exam Chart</h3>
            ...
            {aggregatePieData.length > 0 && (
              <div className="p-4 bg-white shadow-md rounded-2xl">
                <h3 className="mb-4 text-lg font-semibold text-center text-gray-700">Overall Performance Chart</h3>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
