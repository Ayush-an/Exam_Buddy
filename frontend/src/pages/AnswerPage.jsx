...
import examLogo from './assets/exam-buddy-logo.PNG';
import welcomeBannerImage from './assets/Backround.png';
import BeginnerImage from './assets/first.png';
import IntermediateImage from './assets/second.png';
import AdvancedImage from './assets/third.png';
import ProAdvancedImage from './assets/fourth.png';

// --- KEY CHANGE: Define Server Base URL ---
...
export default function AnswerPage() {
    ...
    useEffect(() => {
        ...
        fetchAllExamData();
        if (storedUser._id) {
            fetchProfileImage(storedUser._id);
        }
    }, [navigate]);
  ...
  try {
    ...
    const handleSectionCardClick = (category, sectionName) => {
        ...
        if (!currentSectionData) {
            toast.error('Selected section data not found.');
            return;
        }
        ...
        if (currentSectionData.sets && currentSectionData.sets.length > 0) {
            ...
            toast.warn('No sets found for this section.');
        }
    };
    ...
    const submitExam = useCallback(async (timeUp = false) => {
       ...
       // setCorrectAnswersCount(currentCorrectAnswersCount);
        ...
        try {
            const response = await API.post('/user/submit-exam-results', examAttemptData, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
            setExamResult({
                score: response.data.examResult.score,
                totalMarksPossible: response.data.examResult.totalMarksPossible,
    ...
    const handleOptionSelect = (questionId, optionLetter) => {
        setUserAnswers(prevAnswers => ({ ...prevAnswers, [questionId]: optionLetter }));
    };

    const resetExamFlow = () => {
        setCurrentPhase('dashboard');
        setQuestions([]); setUserAnswers({}); setExamResult(null); setError(null);
        setSelectedExam({ category: '', section: '', set: '', timeLimitMinutes: 0 });
        setAvailableSets([]); clearInterval(timerIntervalRef.current);
        setTimeLeft(0); setStartTime(null); //setCorrectAnswersCount(0);
    };
    ...
    const MediaDisplay = ({ src, type, alt, className }) => {
        ...
        const fullSrc = src.startsWith('http') ? src : `${SERVER_BASE_URL}${src}`;
        const handleError = (e) => {
        ...
        <div className="w-full">
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="items-center hidden space-x-3 md:flex">
                    <div className="text-right">
                        <p className="text-gray-700 dark:text-gray-900">{user?.firstName || 'Guest'}</p>
                        <p className="text-sm text-gray-800 dark:text-gray-400">Student</p>
                    </div>
                    {/* The profileImage state now holds the full URL, so this works correctly */}
                    {profileImage ? (
                        <img src={profileImage} alt="Profile" className="object-cover w-12 h-12 border-2 border-blue-500 rounded-full" />
                    ) : (
                        <div className="flex items-center justify-center w-12 h-12 bg-blue-200 rounded-full dark:bg-blue-800">
                            <FaUserCircle className="text-3xl text-blue-800 dark:text-blue-200" />
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="relative w-full h-56 max-w-full p-8 mb-8 overflow-hidden text-white shadow-lg rounded-xl"
                ...
                <div className="relative z-10">
                    ...
                    <h2 className="mt-2 text-3xl font-bold">Welcome back, {user?.firstName || 'Guest'}!</h2>
                    <p className="mt-1 text-sm">Always stay updated in your student portal</p>
                </div>
            </div>
          {/* Category Tabs */}
<div className="flex pb-4 my-8 space-x-4 overflow-x-auto scrollbar-hide">
  {allExamData.map(category => {
    const categoryColorMap = {
      Beginner: 'bg-pink-400',
      Intermediate: 'bg-pink-400',
      Advanced: 'bg-pink-400',
      'Pro Advanced': 'bg-purple-600',
    };
    ...
    return (
      <motion.button
        ...
        onClick={() => handleCategoryTabChange(category.category)}
...
<h2 className="mb-6 text-2xl font-bold text-gray-800 dark:text-gray-100">
  {activeTab} Study Sections
</h2>

<div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
  {allExamData.find(cat => cat.category === activeTab)?.sections.map(section => {
    const isDisabled = !isUserPaid && !isSectionAccessibleForFreeUser(activeTab, section.name);

    const name = section.name.toLowerCase();

    let sectionImage = IntermediateImage;
    let bgColorClass = 'bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700';

    if (name.includes('beginner')) {
      sectionImage = BeginnerImage;
      bgColorClass = 'bg-green-100 hover:bg-green-200 dark:bg-green-900 dark:hover:bg-green-800';
    } else if (name.includes('intermediate')) {
      sectionImage = IntermediateImage;
      bgColorClass = 'bg-blue-100 hover:bg-blue-200 dark:bg-blue-900 dark:hover:bg-blue-800';
    } else if (name.includes('advanced') && !name.includes('pro')) {
      sectionImage = AdvancedImage;
      bgColorClass = 'bg-yellow-100 hover:bg-yellow-200 dark:bg-yellow-900 dark:hover:bg-yellow-800';
    } else if (name.includes('pro advanced')) {
      sectionImage = ProAdvancedImage;
      bgColorClass = 'bg-purple-100 hover:bg-purple-200 dark:bg-purple-900 dark:hover:bg-purple-800';
    }

    const cardClass = isDisabled
      ? `border-gray-300 text-gray-500 cursor-not-allowed opacity-70 ${bgColorClass}`
      : `border-gray-200 cursor-pointer ${bgColorClass}`;
    ...
    return (
      <motion.div
        ...
        onClick={() => !isDisabled && handleSectionCardClick(activeTab, section.name)}
      ...
      >
        <img
          src={sectionImage}
          alt={section.name}
          className="object-contain w-24 h-24 mb-4 drop-shadow-md"
        />
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white">
          {section.name}
        </h3>
        {isDisabled && <p className="mt-1 text-sm text-red-500">(Paid Access)</p>}
      </motion.div>
    );
  })}
</div>
...
>
  <div className="text-center">
    <h2 className="text-3xl font-extrabold text-indigo-800 dark:text-indigo-200">
      Choose Your Exam Set
    </h2>
    <p className="mt-2 text-lg text-gray-700 dark:text-gray-300">
      Category: <span className="font-semibold text-indigo-600">{selectedExam.category}</span>
    </p>
    <p className="text-lg text-gray-700 dark:text-gray-300">
      Section: <span className="font-semibold text-indigo-600">{selectedExam.section}</span>
    </p>
  </div>
  ...
  {availableSets.length > 0 ? (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      {availableSets.map((set) => (
        ...
        >
          <h3 className="text-xl font-bold text-blue-800 dark:text-blue-100">{set.name}</h3>
          <p className="text-sm text-blue-600 dark:text-blue-300">{set.timeLimitMinutes} minutes</p>
        </motion.button>
      ))}
    </div>
  ) : (
    <p className="text-center text-gray-600 dark:text-gray-400">No sets available for this section.</p>
  )}

  <button
    onClick={resetExamFlow}
    className="w-full px-6 py-3 mt-6 font-semibold text-white transition duration-200 rounded-lg bg-gradient-to-r from-gray-600 to-gray-800 hover:from-gray-700 hover:to-gray-900 dark:from-gray-700 dark:to-gray-900"
  >
    ...
    const renderConfirmExamStart = () => (
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -50 }} transition={{ duration: 0.3 }}
            ...
            <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">Confirm Exam Details</h2>
            <p className="text-lg text-gray-700 dark:text-gray-300">You are about to start the exam for:</p>
            ...
            <button
                onClick={() => setCurrentPhase('selectSet')}
                className="w-full px-6 py-3 mt-4 font-medium text-gray-800 transition-colors duration-200 bg-gray-300 rounded-md hover:bg-gray-400 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
            >
                Go Back
            </button>
        </motion.div>
    ...
    const renderExamInProgress = () => (
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -50 }} transition={{ duration: 0.3 }}
            ...
            <h2 className="mb-4 text-3xl font-bold text-center text-gray-800 dark:text-gray-100">
                Exam: {selectedExam.set} ({selectedExam.category} &gt; {selectedExam.section})
            </h2>
            <div className="mb-6 font-mono text-lg text-right text-gray-700 dark:text-gray-300">
                Time Left: <span className="font-bold text-red-600">{formatTime(timeLeft)}</span>
            </div>
            {error && <p className="text-center text-red-600">{error}</p>}
            <div className="space-y-6">
                {questions.map((question, qIndex) => (
                    <motion.div key={question._id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: qIndex * 0.05 }}
                        ...
                        <div className="space-y-2">
                            {question.options.map((option, oIndex) => {
                                ...
                                return (
                                    <label key={oIndex} className="flex items-center p-3 space-x-3 rounded-md cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600">
                                        <input
                                            type="radio"
                                            name={`question-${question._id}`}
                                            value={optionChar}
                                            checked={userAnswers[question._id] === optionChar}
                                            onChange={() => handleOptionSelect(question._id, optionChar)}
                                        ...
                                        {option.type === 'text' ? (
                                            <span className="text-gray-700 dark:text-gray-200">{option.content}</span>
                                        ) : (
    ...
    const renderExamSubmitted = () => (
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -50 }} transition={{ duration: 0.3 }}
            className="w-full max-w-xl p-8 space-y-6 text-center bg-white rounded-lg shadow-lg dark:bg-gray-800 dark:text-gray-100">
            <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">Exam Completed!</h2>
            {error && <p className="mb-4 text-red-600">{error}</p>}
            {examResult && (
                <div className="space-y-3 text-lg text-gray-700 dark:text-gray-300">
                    <p>Your Score: <span className="font-bold text-blue-700">{examResult.score} / {examResult.totalMarksPossible}</span></p>
            ...
            <button onClick={resetExamFlow} className="w-full px-4 py-3 mt-4 font-medium text-gray-800 bg-gray-300 rounded-md hover:bg-gray-400 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600">
                Start New Exam
            </button>
        </motion.div>
    );
    ...
    return (
        <div className={`flex min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
            {/* ... rest of the main return statement (loading overlay, sidebar, main content) ... */}
            {loading && (
            ...
            <button className="fixed z-50 p-2 bg-white rounded-full shadow-lg top-4 left-4 dark:bg-gray-700 md:hidden" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
                {isSidebarOpen ? <FaTimes className="text-gray-700 dark:text-gray-100" /> : <FaBars className="text-gray-700 dark:text-gray-100" />}
            </button>
                    ...
                    >
                        <div>
                            <div className="flex items-center mb-10 ml-2">
                                <img src={examLogo} alt="Exam Buddy Logo" className="w-10 h-10 mr-2" />
                                <span className="text-2xl font-extrabold text-gray-800 dark:text-gray-100">EXAM.BUDDY</span>
                            </div>
                            <nav className="space-y-4">
                                ...
                                <Link to="#" className={`flex items-center p-3 rounded-lg text-gray-700 dark:text-gray-300 font-medium transition-colors duration-200 ${currentPhase === 'dashboard' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900 dark:text-indigo-200' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                                    onClick={() => { setCurrentPhase('dashboard'); if (window.innerWidth < 768) setIsSidebarOpen(false); }}>
                                    <FaChartLine className="w-5 h-5 mr-3" /> Dashboard
                                </Link>
                                <Link to="/profile" className="flex items-center p-3 font-medium text-gray-700 transition-colors duration-200 rounded-lg dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700" onClick={() => { if (window.innerWidth < 768) setIsSidebarOpen(false); }}>
                                    <FaUserCircle className="w-5 h-5 mr-3" />profile
                                </Link>
                                <div className="relative">
                                    <button className={`w-full flex items-center justify-between p-3 rounded-lg text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200`}
                                        onClick={() => setIsExaminationDropdownOpen(prev => !prev)}>
                                        <div className="flex items-center">
                                            <FaTasks className="w-5 h-5 mr-3" /> Examination
                                        </div>
                                        {isExaminationDropdownOpen ? <FaChevronUp className="w-4 h-4" /> : <FaChevronDown className="w-4 h-4" />}
                                    </button>
                                    <AnimatePresence>
                                ...
                                <Link to="/subscription" className="flex items-center p-3 font-medium text-gray-700 transition-colors duration-200 rounded-lg dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700" onClick={() => { if (window.innerWidth < 768) setIsSidebarOpen(false); }}>
                                    <FaCube className="w-5 h-5 mr-3" /> Subscription
                                </Link>
                                ...
                                <Link to="/support" className="flex items-center p-3 font-medium text-gray-700 transition-colors duration-200 rounded-lg dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700" onClick={() => { if (window.innerWidth < 768) setIsSidebarOpen(false); }}>
                                    <FaLifeRing className="w-5 h-5 mr-3" /> Support
                                </Link>
                            </nav>
                        </div>
                        <div className="flex items-center justify-center h-40">
                            <Lottie animationData={animationData} loop={true} autoplay={true} style={{ width: '100%', maxWidth: '180px', height: 'auto' }} />
                        </div>
                        <div className="pt-4 mt-auto border-t border-gray-200 dark:border-gray-700">
                            <button onClick={handleLogout} className="flex items-center justify-center w-full p-3 text-red-600 transition-colors duration-200 border border-red-600 rounded-full hover:bg-red-50 dark:hover:bg-red-900 dark:text-red-300 dark:border-red-300">
                                <FaPowerOff className="w-5 h-5 mr-2" /> Logout
                            </button>
                            <button onClick={toggleDarkMode} className="flex items-center justify-center w-full p-3 mt-2 text-gray-700 transition-colors duration-200 border border-gray-300 rounded-full hover:bg-gray-100 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-700">
                                {darkMode ? <FaToggleOn className="w-5 h-5 mr-2" /> : <FaToggleOff className="w-5 h-5 mr-2" />}
...
<section
    className={`flex items-center justify-center w-full min-h-full p-6 md:p-12 overflow-hidden
    bg-gradient-to-br from-purple-600 via-purple-500 to-purple-600 rounded-3xl shadow-2xl dark:bg-gray-800 dark:bg-opacity-80 dark:border-gray-700 backdrop-blur-lg`}>
                       <AnimatePresence mode="wait">
                        <motion.div
                            ...
                            className="w-full"
                        >
                            {currentPhase === 'dashboard' && renderDashboard()}
                            {currentPhase === 'selectSet' && renderSelectSet()}
                            {currentPhase === 'confirmExamStart' && renderConfirmExamStart()}
